...
`ProductComment` gana un `moderationStatus` (`PENDING` al publicarse, y uno de `APPROVED`/`DELETED`/`HIDDEN`/`EDITED`/`MARKED` tras la acción correspondiente, uno a uno). HU-41 no declara ninguna transición vetada entre esos cinco estados: un comentario oculto puede aprobarse después.

### Eliminar es borrado físico, no lógico (HU-41.9, Management#29)

El PDF fuente (7.3.3) precisa la acción `DELETE` como "remover permanentemente el comentario del sistema". La implementación original de HU-41 hacía un borrado lógico (`moderationStatus = DELETED`, la fila permanecía) — una diferencia real entre implementación y fuente funcional, corregida en HU-41.9: `DeleteComment` ahora elimina físicamente la fila de `product_comments` (`ProductCommentRepositoryPort.deleteById`).

La evidencia **no desaparece**: `CommentModerationAction` (actor, fecha, motivo, estado anterior, estado nuevo lógico `DELETED`, IP) y cualquier `CommentReport` sobre ese comentario siguen existiendo — ninguna de las dos tablas tiene clave foránea hacia `product_comments`, por diseño, precisamente para sobrevivir a este borrado. El registro de auditoría se construye a partir de la instantánea del comentario ANTES de borrarlo, dentro de la misma transacción (`CommentModerationTransactionPort`): o se borra la fila y se escribe la auditoría juntos, o no se hace ninguna de las dos cosas.

El contrato HTTP **no cambia**: `POST /comments/:commentId/deletion` sigue devolviendo `ProductCommentResponse` con `moderationStatus: DELETED`, tomado de la instantánea en memoria antes del borrado, no de una relectura de la fila (que ya no existiría). La cola de moderación (`ListModerationQueue`) ya toleraba un comentario ausente para cualquier reporte/señal huérfanos; ese es exactamente el caso que ahora ocurre en la práctica cuando un comentario reportado o señalado se elimina.
